...
public class example_ui_color_animation_screenHandle : MonoBehaviour
{
	...
	void button_click( PointerEventData eventData , UI_Event ev)
	{
		if(!isopen)
		{
			exampleImage.Play("open-color",on_animator_finish);
			isopen = true;
		}
		else
		{
			exampleImage.Play("close-color",on_animator_finish);
			isopen = false;
		}
	}
